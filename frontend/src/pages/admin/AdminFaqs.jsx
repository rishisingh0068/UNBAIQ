...
const AdminFaqs = () => {
  ...
  return <section className="mx-auto max-w-[1500px]">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-semibold text-[#063d6b]">FAQs</h2><p className="mt-1 text-sm text-[#667d90]">Manage questions shared across website FAQ sections.</p></div><NavLink to="/admin/faqs/new" className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#063d6b] px-4 py-3 text-sm font-semibold text-white hover:bg-[#0a527f]"><Plus size={18} /> Add new FAQ</NavLink></header>
    {error && <p className="mt-5 rounded-lg bg-red-50 p-4 text-sm text-red-700">{error}</p>}
    {isLoading ? <p className="mt-7 text-sm text-[#667d90]">Loading FAQs...</p> : faqs.length === 0 ? <div className="mt-7 rounded-xl border border-dashed border-[#b9cedc] bg-white p-10 text-center"><HelpCircle className="mx-auto text-[#8496a5]" /><p className="mt-3 text-sm text-[#667d90]">No FAQs found.</p></div> : <div className="mt-7 overflow-hidden rounded-xl border border-[#dce5ec] bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm">
