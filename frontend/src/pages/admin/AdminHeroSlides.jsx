...
const AdminHeroSlides = () => {
  ...
  return <section className="mx-auto max-w-[1500px]">
    {/* Mobile actions remain reachable while the detailed table scrolls horizontally. */}
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-semibold text-[#063d6b]">Hero Slider</h2><p className="mt-1 text-sm text-[#667d90]">Manage slide content, publishing and display order.</p></div><NavLink to="/admin/hero-section/new" className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#063d6b] px-4 py-3 text-sm font-semibold text-white hover:bg-[#0a527f] sm:w-auto"><Plus size={18} /> Add new slide</NavLink></header>
    {error && <p className="mt-5 rounded-lg bg-red-50 p-4 text-sm text-red-700">{error}</p>}
    {isLoading ? <p className="mt-7 text-sm text-[#667d90]">Loading hero slides...</p> : <div className="mt-7 overflow-hidden rounded-xl border border-[#dce5ec] bg-white shadow-sm"><div className="overflow-x-auto overscroll-x-contain"><table className="w-full min-w-[850px] text-left text-sm">
